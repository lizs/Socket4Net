...
namespace socket4net
{
    ...
    internal class WebsocketDelegate<THost> where THost : IWebsocketDelegateHost
    {
        ...
        /// </summary>
        protected Func<byte[], IDataProtocol> DataParser { get; set; } =
            data => PbSerializer.Deserialize<DefaultDataProtocol>(data);
        ...
        public async Task<RpcResult> BroadcastAsync(byte[] data)
        {
            ...
            return
                await
                    TaskHelper.WrapCallback<byte[], RpcResult>(data,
                        (bytes, cb) => { serverHost.BroadcastAsync(bytes, () => { cb(RpcResult.Success); }); });
        }
